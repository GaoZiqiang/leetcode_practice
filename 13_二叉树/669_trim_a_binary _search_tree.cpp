 ...
 * 原题链接：https://leetcode-cn.com/problems/trim-a-binary-search-tree/
 * */
...
/* 方法一：基于递归
 * 核心思想：
 * 若root->data > R：太大--找一个小一点的-->转左子树(左小右大)
 * 若root->data < L：太小--找一个大一点的-->转右子树(左小右大)
 *
 * 复杂度分析：
...
BiTNode* trimBST(BiTree& root, int L, int R) {
    if (!root)
        return root;// root为nullptr则返回nullptr

    // 根节点值判断
    if (root->data > R)// 太大--找一个小一点的-->转左子树(左小右大)
        return trimBST(root->lchild, L, R);
    if (root->data < L)// 太小--找一个大一点的-->转右子树(左小右大)
